...
bit g_bRunDdcciExchangeCommand = _FAIL;
BYTE g_ucDdcciExchangeCountEXINT;
BYTE g_pucDdcciExchangeRxBuf[_TWO_CHIP_DDCCI_EXCHANGE_RXBUF_LENGTH];
#endif
...
void ScalerMcuFlash2ClockDiv(EnumFlash2ClkDiv enumClkDiv);
...
//--------------------------------------------------
// Description  : Set Flash Clock Divider2.
// Input Value  : ClkDiv --> Flash Clock Divider
// Output Value : None
//--------------------------------------------------
void ScalerMcuFlash2ClockDiv(EnumFlash2ClkDiv enumClkDiv)
{
    enumClkDiv &= 0x0F;
    enumClkDiv |= (MCU_FFE9_MCU_CLK_CONTROL_1 & 0xF0);
    MCU_FFE9_MCU_CLK_CONTROL_1 = enumClkDiv;
}
...
void ScalerMcuPwmAdjustFrequency(BYTE ucPwmType, WORD usFrequency)
{
    ...
    if((MCU_FF3A_PWM_CK_SEL & ucPwmType) == ucPwmType)
    {
        for(ucPWM_M = 0; ucPWM_M < 4; ucPWM_M++)
        {
            ...
            // RL6463 and RL6492 HW Using M2Pll / 2 as PWM Reference Clock Source
...
void ScalerMcu5usDelayCalc(EnumClkSel enumClock)
{
    ...
    if((MCU_FDA0_CACHE_FUNCTION & _BIT0) == 0x00)
    {
    ...
    else
    {
        ...
        if((WORD)_HW_IIC_DELAY_TIME * g_ucDelay5usN > 40)
        {
            ...
            if(enumClock == _M2PLL_CLK)
            {
                g_ucDelay5usN = (((WORD)(_HW_IIC_DELAY_TIME * g_ucDelay5usN + 300) + 6) / 12);
            }
            else
#endif
            {
