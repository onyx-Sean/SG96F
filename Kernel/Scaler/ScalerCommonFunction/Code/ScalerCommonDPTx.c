...
void ScalerDpTxSSTSetting(void)
{
    ...
    usMeasureTime = (WORD)(PDATA_DWORD(1) / ((GET_DP_TX_DOWNSTREAM_DOWN_SPREAD() == _TRUE) ? ((DWORD)27 * GET_DP_TX_DOWNSTREAM_LINK_RATE() * 9975 / 10000) : ((DWORD)27 * GET_DP_TX_DOWNSTREAM_LINK_RATE())) / 5);
    usMeasureTime *= 2;
...
BYTE ScalerDpTxTrainPattern1(void)
{
    ...
    if((pData[0] != 0x00) && (GET_DP_TX_DOWNSTREAM_DPCD_REV() == _DP_VERSION_1_2))
    {
        // Delay 4*pData[0] ms
        ScalerTimerDelayXms(pData[0] << 2);
    }
...
#elif(_AUX_TALK_MODE_LDO_VOL_TYPE == _AUX_LDO_VOL_GEN_2_HBR3)
        // Set Tx VLDO = 0.3V, For DP1.4 Aux Diff Peak to Peak = 0.29 to 0.4 V
        ScalerSetBit(PBD_61_AUX_1, ~(_BIT7 | _BIT6 | _BIT5), 0x00);
#endif
        // Set ADJR P
