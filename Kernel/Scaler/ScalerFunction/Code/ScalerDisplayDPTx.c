...
bit ScalerDisplayDPTx1LinkConfig(void)
{
    ...
    {
        if(pData[1] >= _DP_LINK_HBR2)
        {
            SET_DISPLAY_DP_TX1_MAX_LINK_RATE(_DP_LINK_HBR2);
        }
        else
        {
...
void ScalerDisplayDPTx1VideoStreamOutput(bit bOutput)
{
    ...
    if(bOutput == _ENABLE)
    {
        ...
        usMeasureTime = (WORD)(PDATA_DWORD(1) / ((GET_DISPLAY_DP_TX_SSCG_CONFIG() == _TRUE) ? ((DWORD)27 * GET_DISPLAY_DP_TX1_LINK_RATE() * 9975 / 10000) : ((DWORD)27 * GET_DISPLAY_DP_TX1_LINK_RATE())) / 5);
        usMeasureTime *= 2;
...
bit ScalerDisplayDPTx2LinkConfig(void)
{
    ...
    if((pData[1] == _DP_LINK_RBR) || (pData[1] == _DP_LINK_HBR) || ((pData[0] >= _DP_VERSION_1_2) && (pData[1] >= _DP_LINK_HBR2)))
    {
        if(pData[1] >= _DP_LINK_HBR2)
        {
            SET_DISPLAY_DP_TX2_MAX_LINK_RATE(_DP_LINK_HBR2);
        }
        else
        {
...
void ScalerDisplayDPTx2VideoStreamOutput(bit bOutput)
{
    ...
    if(bOutput == _ENABLE)
    {
        ...
        // Calculate HW measure Mvid period time, / 5 for get 5us times
        usMeasureTime = (WORD)(PDATA_DWORD(1) / ((GET_DISPLAY_DP_TX_SSCG_CONFIG() == _TRUE) ? ((DWORD)27 * GET_DISPLAY_DP_TX2_LINK_RATE() * 9975 / 10000) : ((DWORD)27 * GET_DISPLAY_DP_TX2_LINK_RATE())) / 5);
        usMeasureTime *= 2;
