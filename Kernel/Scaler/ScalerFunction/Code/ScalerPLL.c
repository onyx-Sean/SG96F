...
void ScalerPLLSetMPLLFreq(DWORD ulFreq)
{
    ...
    // MPLL_VCO_clk_now
    PDATA_DWORD(0) = ulFreq * 16 / 15 * _MPLL_FIXED_DIV;
    ...
    PDATA_DWORD(0) = (DWORD)usMcode * 700UL * 628 * 20 / PDATA_DWORD(0) / ((7 + (ScalerGetBit(P0_F7_MPLL_CRNT, (_BIT7 | _BIT6 | _BIT5)) >> 5) * 2) * 100);
...
void ScalerPLLDDR2PLLCtrl(bit bEnable)
{
    ...
    for(ucDDRSel = _DDR2_DEV_SEQ_BEGIN; ucDDRSel < _DDR2_DEV_SEQ_END; ucDDRSel++)
    {
        if(bEnable == _ENABLE)
        {
            ...
            for(ucDlyCount = 0; ucDlyCount < 20; ucDlyCount++)
            {
                Delay5us(g_ucDelay5usN);
            }
            ...
            for(ucDlyCount = 0; ucDlyCount < 10; ucDlyCount++)
            {
                Delay5us(g_ucDelay5usN);
            }
        }
...
void ScalerPLLDDR2PLLOutputCtrl(bit bEnable)
{
    ...
    for(ucDDRSel = _DDR2_DEV_SEQ_BEGIN; ucDDRSel < _DDR2_DEV_SEQ_END; ucDDRSel++)
    {
        ...
        {
            // Enable DDR2PLL CLK
            ScalerSetDWord(DDR2_GET_REG(P88_04_PLL_CTRL0_1, ucDDRSel), 0x00CF1FFF);
        ...
        {
            // Disable DDR2PLL CLK
            ScalerSetDWord(DDR2_GET_REG(P88_04_PLL_CTRL0_1, ucDDRSel), 0x00001FFF);
