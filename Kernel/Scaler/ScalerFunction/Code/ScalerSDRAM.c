...
void ScalerSdramSearchPhaseCheck(void)
{
    if((ScalerGetByte(P4_C3_SDR_RX_CLK_DLY2) == 0x00) && (ScalerGetByte(P4_C5_SDR_RX_CLK_DLY4) == 0x00))
    {
        // Tx Clk Search Phase
        ScalerSdramTxClkSearchPhase();
...
void ScalerSdramTxClkSearchPhase(void)
{
    ...
    if((ucBestPhase >= g_pucSdramDelayChain[0]) && (ucTempDiffPhase >= (g_pucSdramDelayChain[0] + (g_pucSdramDelayChain[0] / 2))))
    {
        ucBestPhase = ucBestPhase / 2;
    }
    else
    {
        if(ucBestPhase >= (g_pucSdramDelayChain[0] / 2))
        {
            if(ucPhase0 == _TRUE)
            {
                ucBestPhase = 0;
            ...
            {
                if(ucTempDiffPhase >= ((g_pucSdramDelayChain[0] / 2) + (g_pucSdramDelayChain[0] / 4)))
                {
                    ucBestPhase = ucBestPhase / 2;
                }
                else
