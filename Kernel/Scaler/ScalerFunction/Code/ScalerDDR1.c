...
void ScalerDDR1TokenRingSet(void)
{
    ...
    // DDR1*2 && FRC on, frc capture has the highest priority
    ScalerSetByte(P4_AC_SDR_RSC_CAP1, 0xFF);
    ScalerSetByte(P4_AE_SDR_RSC_MAIN, 0x11);
#else
...
void ScalerDDR1SearchPhaseCheck(void)
{
    if((ScalerGetByte(P4_D0_DDR_RX_DLY1) == 0x00) && (ScalerGetByte(P4_D1_DDR_RX_DLY2) == 0x00) &&
       (ScalerGetByte(P4_D2_DDR_RX_DLY3) == 0x00) && (ScalerGetByte(P4_D3_DDR_RX_DLY4) == 0x00))
    {
        // Tx Clk Search Phase
        ScalerDDR1TxClkSearchPhase();
...
void ScalerDDR1TxClkSearchPhase(void)
{
    ...
    if((ucBestPhase >= g_pucDDR1DelayChain[0]) && (ucTempDiffPhase >= (g_pucDDR1DelayChain[0] + (g_pucDDR1DelayChain[0] / 2))))
    {
        pData[3] = (ucBestPhase / 2);
    }
    else
    {
        if(ucBestPhase >= (g_pucDDR1DelayChain[0] / 2))
        {
            if(ucPhase0 == _TRUE)
            {
                pData[3] = 0;
            ...
            {
                if(ucTempDiffPhase >= ((g_pucDDR1DelayChain[0] / 2) + (g_pucDDR1DelayChain[0] / 4)))
                {
                    pData[3] = (ucBestPhase / 2);
                }
                else
...
//**********************************************************************************//
    ...
    if((ucBestPhase >= g_pucDDR1DelayChain2[0]) && (ucTempDiffPhase >= (g_pucDDR1DelayChain2[0] + (g_pucDDR1DelayChain2[0] / 2))))
    {
        pData[3] = (ucBestPhase / 2);
    }
    else
    {
        if(ucBestPhase >= (g_pucDDR1DelayChain2[0] / 2))
        {
            if(ucPhase0 == _TRUE)
            {
                pData[3] = 0;
            ...
            {
                if(ucTempDiffPhase >= ((g_pucDDR1DelayChain2[0] / 2) + (g_pucDDR1DelayChain2[0] / 4)))
                {
                    pData[3] = (ucBestPhase / 2);
                }
                else
