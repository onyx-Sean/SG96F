...
void ScalerTMDSRx0GetHDRData(BYTE *pucHDRData);
#endif
...
void ScalerTMDSHdmiPacketDetect(bit bWait)
{
    if(bWait == _WAIT)
    {
        ...
        {
            SET_HDMI_RX0_HDR_INFO_FRAME_RECEIVED();
        }
        else
        {
            CLR_HDMI_RX0_HDR_INFO_FRAME_RECEIVED();
        }
#endif
...
void ScalerTMDSRx0GetHDRData(BYTE *pucHDRData)
{
    ScalerGetDataPortByte(P2_CD_HDMI_PSAP, _HDR_INFO_HDMI_VERSION, 2, &pucHDRData[0], _AUTOINC);
    ScalerGetDataPortByte(P2_CD_HDMI_PSAP, _HDR_INFO_HDMI_EOTF, 2, &pucHDRData[2], _AUTOINC);
    ScalerGetDataPortByte(P2_CD_HDMI_PSAP, _HDR_INFO_HDMI_DISPLAY_PRIMARIES_X0_LSB, 24, &pucHDRData[4], _AUTOINC);
}
