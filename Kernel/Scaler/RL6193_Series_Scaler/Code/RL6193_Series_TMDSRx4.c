...
#define __RL6193_SERIES_TMDSRX4__
...
void ScalerTMDSRx4SetPhy_EXINT0(void) using 1
{
    ...
    if(ScalerGetBit_EXINT(PB3_A0_CMU_00, _BIT6) == _BIT6)   // MHL Mode
    {
        ...
        else  // MHL 24bit Mode
        {
            ...
            if((GET_INPUT_PIXEL_CLK() >= 700) && (GET_INPUT_PIXEL_CLK() < 1125))
            {
                ...
                ScalerSetBit_EXINT(PB3_A6_CMU_06, ~_BIT6, _BIT6);
            }
            ...
            else if(GET_INPUT_PIXEL_CLK() < 3000)
            {
                ...
                ScalerSetBit_EXINT(PB3_FD_LANE0_A_CDR_07, ~(_BIT4 | _BIT3 | _BIT2), (_BIT3 | _BIT2));
...
#endif
    {
        ...
        else if((GET_INPUT_PIXEL_CLK() >= 375) && (GET_INPUT_PIXEL_CLK() < 750))
        {
            ...
            // [4:2] CMU_PI_I_SEL = 3'b100 --> 15.625uA
            // [1:0] CMU_SELPREDIV = 2'b00 --> /1
            ScalerSetByte_EXINT(PB3_A3_CMU_03, 0x70);
        ...
        else if((GET_INPUT_PIXEL_CLK() >= 750) && (GET_INPUT_PIXEL_CLK() < 1500))
        {
            ...
            ScalerSetByte_EXINT(PB3_F2_SDM_01, 0x10);
        }
...
#endif
    {
        ...
        {
            // KP = 0x20 --> KP = 8
            ScalerSetByte_EXINT(PB3_D6_KP_0, 0x20);
            ScalerSetByte_EXINT(PB3_D7_KP_1, 0x20);
            ScalerSetByte_EXINT(PB3_D8_KP_2, 0x20);
        }
    ...
    if((ScalerGetBit_EXINT(PB3_A0_CMU_00, _BIT6) == _BIT6) &&
       (((GET_TMDS_RX4_CLK_MODE() == _TMDS_MHL_PPMODE_PHY_SETTING)) ||
        ((GET_TMDS_RX4_CLK_MODE() == _TMDS_24BIT_PHY_SETTING) && (GET_INPUT_PIXEL_CLK() >= 2000))))
    {
        // [7] Enable Lane0 ACDR
...
bit ScalerTMDSRx4CheckDataDFE(void)
{
    ...
    if(ScalerGetBit(PB3_A0_CMU_00, _BIT6) != _BIT6)   // HDMI MODE
    {
...
void ScalerTMDSRx4HotPlugEvent(void)
{
    ...
    {
        // Save Z0 Status
        ucZ0Status = ScalerGetBit(PB3_A2_CMU_02, (_BIT7 | _BIT6 | _BIT5 | _BIT4));

        // Backup Slave Address
        ScalerGetDataPortByte(P73_C3_HDCP_AP, _P73_C4_PT_C2_PORT_PAGE73_HDCP_SLAVE_ADD, 1, &ucSlaveAddr, _NON_AUTOINC);
...
#if(_D4_INPUT_PORT_TYPE == _D4_DUAL_DVI_PORT)
        // Save Z0 Status
        ucDualZ0Status = ScalerGetBit(PB4_A2_CMU_02, (_BIT7 | _BIT6 | _BIT5 | _BIT4));
...
#endif
        {
            ...
            ScalerSetDataPortByte(P73_C3_HDCP_AP, _P73_C4_PT_C2_PORT_PAGE73_HDCP_SLAVE_ADD, ucSlaveAddr);
        }

        // Restore Z0 Status
        ScalerSetBit(PB3_A2_CMU_02, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), ucZ0Status);

#if(_D4_INPUT_PORT_TYPE == _D4_DUAL_DVI_PORT)
        // Restore Z0 Status
        ScalerSetBit(PB4_A2_CMU_02, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), ucDualZ0Status);
#endif
