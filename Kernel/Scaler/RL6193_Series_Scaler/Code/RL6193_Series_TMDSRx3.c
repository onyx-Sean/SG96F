...
#define __RL6193_SERIES_TMDSRX3__
...
void ScalerTMDSRx3SetPhy_EXINT0(void) using 1
{
    ...
    if(ScalerGetBit_EXINT(PB2_A0_CMU_00, _BIT6) == _BIT6)   // MHL Mode
    {
        ...
        if(GET_TMDS_RX3_CLK_MODE() == _TMDS_MHL_PPMODE_PHY_SETTING)
        {
            ...
            // [4:2] reg_sel_cdr_res = 3'b101 --> Analog CDR Rs = 2K
        ...
        else  // MHL 24bit Mode
        {
            ...
            if((GET_INPUT_PIXEL_CLK() >= 700) && (GET_INPUT_PIXEL_CLK() < 1125))
            {
                ...
                ScalerSetBit_EXINT(PB2_A6_CMU_06, ~_BIT6, _BIT6);
            }
            ...
            else if(GET_INPUT_PIXEL_CLK() < 3000)
            {
                ...
                // [4:2] reg_sel_cdr_res = 3'b101 --> Analog CDR Rs = 4K
                ScalerSetBit_EXINT(PB2_FD_LANE0_A_CDR_07, ~(_BIT4 | _BIT3 | _BIT2), (_BIT3 | _BIT2));
...
#endif
    {
        ...
        else if((GET_INPUT_PIXEL_CLK() >= 375) && (GET_INPUT_PIXEL_CLK() < 750))
        {
            ...
            // [4:2] CMU_PI_I_SEL = 3'b100 --> 15.625uA
            // [1:0] CMU_SELPREDIV = 2'b00 --> /1
            ScalerSetByte_EXINT(PB2_A3_CMU_03, 0x70);
        ...
        else if((GET_INPUT_PIXEL_CLK() >= 750) && (GET_INPUT_PIXEL_CLK() < 1500))
        {
            ...
            ScalerSetByte_EXINT(PB2_F2_SDM_01, 0x10);
        }
...
#endif
    {
        ...
        {
            // KP = 0x20 --> KP = 8
            ScalerSetByte_EXINT(PB2_D6_KP_0, 0x20);
            ScalerSetByte_EXINT(PB2_D7_KP_1, 0x20);
            ScalerSetByte_EXINT(PB2_D8_KP_2, 0x20);
        }
    ...
    if((ScalerGetBit_EXINT(PB2_A0_CMU_00, _BIT6) == _BIT6) &&
       (((GET_TMDS_RX3_CLK_MODE() == _TMDS_MHL_PPMODE_PHY_SETTING)) ||
        ((GET_TMDS_RX3_CLK_MODE() == _TMDS_24BIT_PHY_SETTING) && (GET_INPUT_PIXEL_CLK() >= 2000))))
    {
        // [7] Enable Lane0 ACDR
    ...
    // Enable Adaptation
    if(GET_INPUT_PIXEL_CLK() > 3000) // 3000~3400
    {
    ...
    else if(GET_INPUT_PIXEL_CLK() > 375) // 375~3000
    {
...
void ScalerTMDSRx3IntHandler_EXINT0(void) using 1
{
    ...
    if(((ScalerGetByte_EXINT(P72_E6_HDMI_FREQDET_CTRL) & 0x06) == 0x06) && (g_ucTMDSRx3ClkIRQ == _ENABLE))
    {
        ...
        {
#if(_D3_INPUT_PORT_TYPE == _D3_DUAL_DVI_PORT)
            if(ScalerDualLinkDVIRx3GetDualDVIMode_EXINT0() == _TRUE)
#endif
            {
...
bit ScalerTMDSRx3CheckDataDFE(void)
{
    ...
    if(ScalerGetBit(PB2_A0_CMU_00, _BIT6) != _BIT6)   // HDMI MODE
    {
...
void ScalerTMDSRx3HotPlugEvent(void)
{
    ...
    {
        // Save Z0 Status
        ucZ0Status = ScalerGetBit(PB2_A2_CMU_02, (_BIT7 | _BIT6 | _BIT5 | _BIT4));

        // Backup Slave Address
        ScalerGetDataPortByte(P72_C3_HDCP_AP, _P72_C4_PT_C2_PORT_PAGE72_HDCP_SLAVE_ADD, 1, &ucSlaveAddr, _NON_AUTOINC);
...
#if(_D3_INPUT_PORT_TYPE == _D3_DUAL_DVI_PORT)
        // Save Z0 Status
        ucDualZ0Status = ScalerGetBit(PB3_A2_CMU_02, (_BIT7 | _BIT6 | _BIT5 | _BIT4));
...
#endif
        {
            ...
            ScalerSetDataPortByte(P72_C3_HDCP_AP, _P72_C4_PT_C2_PORT_PAGE72_HDCP_SLAVE_ADD, ucSlaveAddr);
        }

        // Restore Z0 Status
        ScalerSetBit(PB2_A2_CMU_02, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), ucZ0Status);

#if(_D3_INPUT_PORT_TYPE == _D3_DUAL_DVI_PORT)
        // Restore Z0 Status
        ScalerSetBit(PB3_A2_CMU_02, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), ucDualZ0Status);
#endif
