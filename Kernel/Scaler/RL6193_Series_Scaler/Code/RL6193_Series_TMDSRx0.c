...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6193_Series_TMDSRx0.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6193_SERIES_TMDSRX0__
...
//****************************************************************************
void ScalerTMDSRx0Initial(void);
...
void ScalerTMDSRx0ReadRequestTimerEventEnable_WDINT(bit bEnable);
void ScalerTMDSRx0Hdmi2FormatResetEvent(void);
#endif
...
bit ScalerTMDSRx0TMDSPSPreDetect(void);
bit ScalerTMDSRx0TMDSNormalPreDetect(void);
...
bit ScalerTMDSRx0CheckDataDFE(void);
...
void ScalerTMDSRx0SetDFEInitial_EXINT0(void) using 1
{
    ...
    if((GET_INPUT_PIXEL_CLK() > 1000) && ((GET_INPUT_PIXEL_CLK() < 3400)))
    {
...
void ScalerTMDSRx0SetPhy_EXINT0(void) using 1
{
    ...
    if(ScalerGetBit_EXINT(PB_A0_CMU_00, _BIT6) == _BIT6)   // MHL Mode
    {
        ...
        else  // MHL 24bit Mode
        {
            ...
            if((GET_INPUT_PIXEL_CLK() >= 700) && (GET_INPUT_PIXEL_CLK() < 1125))
            {
                ...
                // [1] HST_BUF_SEL = 1'b1 --> Enable Hysteresis Amplifier in Clock Lane
                ScalerSetBit_EXINT(PB_A6_CMU_06, ~_BIT6, _BIT6);
            }
            ...
            else if((GET_INPUT_PIXEL_CLK() >= 1500) && (GET_INPUT_PIXEL_CLK() < 2360))
            {
...
#endif
    {
...
#endif
        {
            ...
            else if((GET_INPUT_PIXEL_CLK() >= 375) && (GET_INPUT_PIXEL_CLK() < 750))
            {
                ...
                // [4:2] CMU_PI_I_SEL = 3'b100 --> 15.625uA
                // [1:0] CMU_SELPREDIV = 2'b00 --> /1
                ScalerSetByte_EXINT(PB_A3_CMU_03, 0x70);
            ...
            else if((GET_INPUT_PIXEL_CLK() >= 3000) && (GET_INPUT_PIXEL_CLK() < 3400))
            {
            ...
            else if((GET_INPUT_PIXEL_CLK() >= 3400) && (GET_INPUT_PIXEL_CLK() < 6300))
            {
                // [7:5] rate_sel = 3'b000 --> 3400MHz ~ 6000MHz
                ScalerSetBit_EXINT(PB_DB_ST_0, ~(_BIT7 | _BIT6 | _BIT5), 0x00);
    ...
    if((GET_INPUT_PIXEL_CLK() >= 2500) && (GET_INPUT_PIXEL_CLK() < 3000))
    {
        // [3:0] KD = 4'b1111 --> KD = 1
...
#endif
    {
        ...
        {
            // KP = 0x20 --> KP = 8
            ScalerSetByte_EXINT(PB_D6_KP_0, 0x20);
            ScalerSetByte_EXINT(PB_D7_KP_1, 0x20);
            ScalerSetByte_EXINT(PB_D8_KP_2, 0x20);
        }
        else if((GET_INPUT_PIXEL_CLK() >= 1500) && (GET_INPUT_PIXEL_CLK() < 2500))
        {
            // KP = 0x0F --> KP = 3.75
        ...
        else if((GET_INPUT_PIXEL_CLK() >= 2500) && (GET_INPUT_PIXEL_CLK() < 3000))
        {
            // KP = 0x2D --> KP = 11.25
    ...
    else if((GET_INPUT_PIXEL_CLK() >= 2500) && (GET_INPUT_PIXEL_CLK() < 3400))
    {
        // [7:5] LE_ISEL = 3'b010
        // [4:2] LE_RLSEL = 3'b001
    ...
    // Enable Adaptation
    if(GET_INPUT_PIXEL_CLK() > 3400)  // 3400~6000 Analog CDR
    {
    ...
    else if(GET_INPUT_PIXEL_CLK() > 2900) // 2900~3400
    {
    ...
    else if(GET_INPUT_PIXEL_CLK() > 375) // 375~2900
    {
        // Disable Analog CDR Mode
        ScalerSetBit_EXINT(PB_B9_LANE0_A_CDR_00, ~_BIT7, 0x00);
    ...
    if(GET_INPUT_PIXEL_CLK() > 3400) // 3400~6000 Hybrid CDR
    {
        // [0] en_eqen_manual = 0 --> Disable Adaptation
        ScalerSetBit_EXINT(PB_F6_EQEN_TIMER, ~_BIT0, 0x00);
...
void ScalerTMDSRx0IntHandler_EXINT0(void) using 1
{
    ...
    if(((ScalerGetByte_EXINT(P2_EF_HDMI_ERROR_TH) & 0xC0) == 0xC0) && (g_ucTMDSRx0ClkIRQ == _ENABLE))
    {
        ...
        if((GET_TMDS_RX0_PHY_SET() == _FALSE) || ((bit)ScalerTMDSRx0CheckCDRStable_EXINT0(g_usTMDSRx0ClkCount) == _FALSE))
        {
            ...
            SET_TMDS_OFFLINE_MEASURE_CLK(((DWORD)(_GDI_CLK_KHZ * 1024) / 100) / g_usTMDSRx0ClkCount);
    ...
    if(((ScalerGetByte_EXINT(P2_E6_HDMI_FREQDET_CTRL) & 0x06) == 0x06) && (g_ucTMDSRx0ClkIRQ == _ENABLE))
    {
...
bit ScalerTMDSRx0HdmiDeepColorProc(WORD usTmdsClkCnt)
{
    ...
    if(ScalerTMDSRx0HdmiSetDeepColorPll(usTmdsClkCnt, GET_COLOR_DEPTH()) == _FALSE)
    {
        return _FALSE;
    }
...
bit ScalerTMDSRx0HdmiSetDeepColorPll(WORD usTMDSClkCount, BYTE ucColorDepth)
{
    ...
    for(pData[15] = 0; pData[15] <= 254; pData[15] += 2)
    {
        ...
        if(((ucN % ucFactor) == 0x00) || ((pData[15] % ucFactor) == 0x00))
        {
            ...
            if((PDATA_DWORD(0) >= 300000) && (PDATA_DWORD(0) <= 750000))//ybm
            {
...
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx0ReadRequestTimerEventEnable_WDINT(bit bEnable) using 3
{
    if(bEnable == _ENABLE)
    {
        if(GET_HDMI_2_0_RX0_RR_ENABLE() == _TRUE)
...
BYTE ScalerTMDSRx0TMDSScanInputPort(BYTE ucInputType)
{
    ...
    if(GET_TMDS_RX0_PHY_SET() == _TRUE)
    {
...
#if(_D0_HDMI_2_0_SUPPORT == _ON)
        if((ScalerTMDSRx0CheckCDRStable(g_usTMDSRx0ClkCount) == _FALSE) || (ScalerTMDSRx0CheckDataDFE() == _FALSE) ||
        ...
        if((ScalerTMDSRx0CheckCDRStable(g_usTMDSRx0ClkCount) == _FALSE) || (ScalerTMDSRx0CheckDataDFE() == _FALSE) || (ScalerTMDSRx0TMDSVideoDetect(_WAIT) == _TMDS_VIDEO_DATA_NONE))
#else
        if((ScalerTMDSRx0CheckCDRStable(g_usTMDSRx0ClkCount) == _FALSE) || (ScalerTMDSRx0CheckDataDFE() == _FALSE))
#endif
        ...
        ScalerSetBit(P2_14_HDMI_CTRL_14, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (GET_INPUT_PIXEL_CLK() / 10) >> 8);
        ScalerSetByte(P2_15_HDMI_CTRL_15, (GET_INPUT_PIXEL_CLK() / 10));
        ...
        if(ucTMDSType == _SOURCE_HDMI)
        {
            ...
            SET_COLOR_DEPTH(ScalerTMDSRx0HdmiGetColorDepth());

            if((GET_COLOR_DEPTH() != _COLOR_DEPTH_8_BITS) && (GET_COLOR_SPACE() != _COLOR_SPACE_YCBCR422))
            {
                DebugMessageRx0("Deep Color Mode Detected", GET_COLOR_DEPTH());
...
bit ScalerTMDSRx0CheckDataDFE(void)
{
    ...
    if(ScalerGetBit(PB_A0_CMU_00, _BIT6) != _BIT6)   // HDMI MODE
    {
...
bit ScalerTMDSRx0TMDSStableDetect(BYTE ucSourceType)
{
    ...
    if(ucSourceType == _SOURCE_HDMI)
    {
        if(GET_HDMI_RX0_PACKET_DETECTING() == _FALSE)
        {
            ...
            if(GET_HDMI_RX0_GCP_RECEIVED() == _TRUE)
            {
                if((GET_COLOR_SPACE() != _COLOR_SPACE_YCBCR422) && (ScalerTMDSRx0HdmiGetColorDepth() != GET_COLOR_DEPTH()))
                {
...
void ScalerTMDSRx0HotPlugEvent(void)
{
...
#endif
    {
        ...
        ScalerGetDataPortByte(P2_C3_HDCP_AP, _P2_C4_PT_C2_PORT_PAGE2_HDCP_SLAVE_ADD, 1, &ucSlaveAddr, _NON_AUTOINC);
...
#endif
        {
            ...
            ScalerSetDataPortByte(P2_C3_HDCP_AP, _P2_C4_PT_C2_PORT_PAGE2_HDCP_SLAVE_ADD, ucSlaveAddr);
        }
