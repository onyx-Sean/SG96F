...
bit ScalerMeasureGetOfflineMeasureResult(StructMeasureSyncData *pstMeasureSyncHV)
{
    ...
    {
        // HSync Period
        pstMeasureSyncHV->usHSPeriod = ((DWORD)ScalerDpGetHSPeriod() * _MEASURE_CLK / 1000000);

        // HSync Pulse Width
        pstMeasureSyncHV->usHSWidth = ScalerDpGetHSW();

        // VSync Period
        pstMeasureSyncHV->usVSPeriod = ScalerDpGetVSPeriod();
    }
    else
#endif
    {
...
bit ScalerMeasureFinalSetting(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort)
{
    ...
    switch(enumSourceSearchPort)
    {
        case _D0_INPUT_PORT:
            ...
            if(ScalerColor420To422D0Recheck() == _FALSE)
            {
        ...
        case _D1_INPUT_PORT:
            ...
            if(ScalerColor420To422D1Recheck() == _FALSE)
            {
...
bit ScalerMeasureInputInfo(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort)
{
    ...
    if((ScalerTimerWaitForEvent(_EVENT_IVS) == _FALSE) ||
       ...
       (ScalerMeasureGetOfflineMeasureResult(&g_stMeasureSyncHV) == _FALSE))
    {
...
void ScalerMeasureUpdateInputTimingPolarity(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort, StructMeasureSyncData *pstMeasureSyncHV)
{
    ...
    switch(enumSourceType)
    {
        ...
        case _SOURCE_VGA:
            ...
            g_stInputTimingData.b1VSP = (pstMeasureSyncHV->b1VSP ^ (bit)ScalerGetBit(P30_01_MEAS_CTL0, _BIT2));
        ...
        case _SOURCE_DP:
            ...
            switch(enumSourceSearchPort)
            {
                ...
                case _D6_INPUT_PORT:
                    ...
                    g_stInputTimingData.b1HSP = (ScalerGetBit(PB6_0E_MSA_HSW_0, _BIT7));
                    g_stInputTimingData.b1VSP = (ScalerGetBit(PB6_18_MSA_VSW_0, _BIT7));
                ...
                case _D1_INPUT_PORT:
                    ...
                    g_stInputTimingData.b1HSP = (ScalerGetBit(PB9_0E_MSA_HSW_0, _BIT7));
                    g_stInputTimingData.b1VSP = (ScalerGetBit(PB9_18_MSA_VSW_0, _BIT7));
        ...
        case _SOURCE_DVI:
            ...
            if(ScalerGetBit((usHdmiPage + P2_A7_TMDS_PWDCTL), _BIT7) == _BIT7)
            {
                if(ScalerGetBit((usHdmiPage + P2_A4_TMDS_CTRL), (_BIT4 | _BIT3)) == (_BIT4 | _BIT3))
                {
                    ...
                    g_stInputTimingData.b1HSP = (ScalerGetBit((usHdmiPage + P2_C0_HDCP_CR), _BIT6) == 0x00);
                    g_stInputTimingData.b1VSP = (ScalerGetBit((usHdmiPage + P2_C0_HDCP_CR), _BIT5) == 0x00);
                }
                else
                {
...
bit ScalerMeasureDualDviOfflineCheck(EnumSourceSearchPort enumSourceSearchPort)
{
    switch(enumSourceSearchPort)
    {
        ...
        case _D4_INPUT_PORT:
            ...
            {
                // Current Timing is not Reasonable, Toggle dual DVI format
                ScalerDualLinkDVIRx3Switch(!ScalerDualLinkDVIRx3GetDualDVIMode());
                return _FALSE;
            }
