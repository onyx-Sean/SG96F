...
bit ScalerDisplayDPTxLinkConfig(void)
{
    ...
    if((pData[1] == _DP_LINK_RBR) || (pData[1] == _DP_LINK_HBR) || ((pData[0] >= _DP_VERSION_1_2) && (pData[1] >= _DP_LINK_HBR2)))
    {
        ...
        {
            SET_DISPLAY_DP_TX_MAX_LINK_RATE(_DP_LINK_HBR2);
        }
        else
        {
...
void ScalerDisplayDPTxVideoStreamOutput(bit bOutput)
{
    ...
    if(bOutput == _ENABLE)
    {
        ...
        usMeasureTime = (WORD)(PDATA_DWORD(1) / ((GET_DISPLAY_DP_TX_SSCG_CONFIG() == _TRUE) ? ((DWORD)27 * GET_DISPLAY_DP_TX_LINK_RATE() * 9975 / 10000) : ((DWORD)27 * GET_DISPLAY_DP_TX_LINK_RATE())) / 5);
        usMeasureTime *= 2;
...
BYTE ScalerDisplayDPTxTrainPattern1(void)
{
    ...
    if(GET_DISPLAY_DP_TX_DPCD_REV() >= _DP_VERSION_1_2)
    {
    ...
    if((pData[0] != 0x00) && (GET_DISPLAY_DP_TX_DPCD_REV() == _DP_VERSION_1_2))
    {
        // Delay 4*pData[0] ms
