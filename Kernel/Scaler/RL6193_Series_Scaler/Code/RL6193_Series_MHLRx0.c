...
bit g_bMHLRx0SentMscDcapRdy = _FALSE;
...
bit ScalerMHLRx0MscHandler(BYTE ucActivePort, BYTE ucModeState);
...
void ScalerMHLRx0TimerEventProc(EnumScalerTimerEventID enumEventID);
...
bit ScalerMHLRx0MscHandler(BYTE ucActivePort, BYTE ucModeState)
{
    ...
    if((ucActivePort == _D0_INPUT_PORT) && (ucModeState != _MODE_STATUS_INITIAL) && (ucModeState != _MODE_STATUS_POWER_OFF))
    {
        switch(GET_MHL_RX0_PROCESS_STATE())
        {
...
//--------------------------------------------------
// Description  : Handle MHL Rx0 Related Timer Event
