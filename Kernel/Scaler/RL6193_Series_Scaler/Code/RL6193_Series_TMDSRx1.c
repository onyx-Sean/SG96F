...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6193_Series_TMDSRx1.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6193_SERIES_TMDSRX1__
...
void ScalerTMDSRx1GetHDRData(BYTE *pucHDRData);
#endif
#if(_ULTRA_HDR_SUPPORT == _ON)
//--------------------------------------------------
...
void ScalerTMDSRx1GetHDRData(BYTE *pucHDRData)
{
    ScalerGetDataPortByte(P70_CD_HDMI_PSAP, _HDR_INFO_HDMI_VERSION, 2, &pucHDRData[0], _AUTOINC);
    ScalerGetDataPortByte(P70_CD_HDMI_PSAP, _HDR_INFO_HDMI_EOTF, 2, &pucHDRData[2], _AUTOINC);
    ScalerGetDataPortByte(P70_CD_HDMI_PSAP, _HDR_INFO_HDMI_DISPLAY_PRIMARIES_X0_LSB, 24, &pucHDRData[4], _AUTOINC);
}
...
#endif
#if(_ULTRA_HDR_SUPPORT == _ON)
    ScalerSetDataPortByte(P70_C9_HDMI_AP, _P71_CA_PT_A5_PORT_PAGE71_HDMI_PTRSV_3, _HDR_INFO_TYPE);
#endif
...
void ScalerTMDSRx1SetPhy_EXINT0(void) using 1
{
    ...
    if(ScalerGetBit_EXINT(PB0_A0_CMU_00, _BIT6) == _BIT6)   // MHL Mode
    {
        ...
        else  // MHL 24bit Mode
        {
            ...
            if((GET_INPUT_PIXEL_CLK() >= 700) && (GET_INPUT_PIXEL_CLK() < 1125))
            {
                ...
                ScalerSetBit_EXINT(PB0_A6_CMU_06, ~_BIT6, _BIT6);
            }
...
#endif
    {
...
#endif
        {
            ...
            else if((GET_INPUT_PIXEL_CLK() >= 375) && (GET_INPUT_PIXEL_CLK() < 750))
            {
                ...
                // [4:2] CMU_PI_I_SEL = 3'b100 --> 15.625uA
                // [1:0] CMU_SELPREDIV = 2'b00 --> /1
                ScalerSetByte_EXINT(PB0_A3_CMU_03, 0x70);
...
#endif
    {
        ...
        {
            // KP = 0x20 --> KP = 8
            ScalerSetByte_EXINT(PB0_D6_KP_0, 0x20);
            ScalerSetByte_EXINT(PB0_D7_KP_1, 0x20);
            ScalerSetByte_EXINT(PB0_D8_KP_2, 0x20);
        }
...
void ScalerTMDSRx1IntHandler_EXINT0(void) using 1
{
    ...
    if(((ScalerGetByte_EXINT(P70_EF_HDMI_ERROR_TH) & 0xC0) == 0xC0) && (g_ucTMDSRx1ClkIRQ == _ENABLE))
    {
    ...
    if(((ScalerGetByte_EXINT(P70_E6_HDMI_FREQDET_CTRL) & 0x06) == 0x06) && (g_ucTMDSRx1ClkIRQ == _ENABLE))
    {
...
void ScalerTMDSRx1HdmiPacketDetect(bit bWait)
{
    if(bWait == _WAIT)
    {
        ...
        if(ScalerTimerPollingDataBitFlagProc(130, P70_C9_HDMI_AP, _P71_CA_PT_00_PORT_PAGE71_HDMI_SCR, _BIT6, _TRUE) == _TRUE)
        {
            SET_HDMI_RX1_HDR_INFO_FRAME_RECEIVED();
        }
        else
        {
            CLR_HDMI_RX1_HDR_INFO_FRAME_RECEIVED();
        }
#endif
...
#endif
#if(_ULTRA_HDR_SUPPORT == _ON)
        if(ScalerGetDataPortBit(P70_C9_HDMI_AP, _P71_CA_PT_00_PORT_PAGE71_HDMI_SCR, _BIT6) == _BIT6)
        {
            SET_HDMI_RX1_HDR_INFO_FRAME_RECEIVED();
        }
        else
...
void ScalerTMDSRx1ReadRequestTimerEventEnable_WDINT(bit bEnable) using 3
{
    ...
    else
    {
        ...
        ScalerSetBit_EXINT(P70_1B_HDMI_CTRL_1B, ~_BIT7, 0x00);
    }
}
...
BYTE ScalerTMDSRx1TMDSScanInputPort(BYTE ucInputType)
{
    ...
    if(GET_TMDS_RX1_PHY_SET() == _TRUE)
    {
        ...
        ScalerSetBit(P70_14_HDMI_CTRL_14, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (GET_INPUT_PIXEL_CLK() / 10) >> 8);
        ScalerSetByte(P70_15_HDMI_CTRL_15, (GET_INPUT_PIXEL_CLK() / 10));
...
bit ScalerTMDSRx1CheckDataDFE(void)
{
    ...
    if(ScalerGetBit(PB0_A0_CMU_00, _BIT6) != _BIT6)   // HDMI MODE
    {
...
void ScalerTMDSRx1HotPlugEvent(void)
{
...
#endif
    {
        ...
        ScalerGetDataPortByte(P70_C3_HDCP_AP, _P70_C4_PT_C2_PORT_PAGE70_HDCP_SLAVE_ADD, 1, &ucSlaveAddr, _NON_AUTOINC);
...
#endif
        {
            ...
            ScalerSetDataPortByte(P70_C3_HDCP_AP, _P70_C4_PT_C2_PORT_PAGE70_HDCP_SLAVE_ADD, 0x7F);
            ...
            ScalerSetDataPortByte(P70_C3_HDCP_AP, _P70_C4_PT_C2_PORT_PAGE70_HDCP_SLAVE_ADD, ucSlaveAddr);
        }
