...
/********************************************************************************/
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6193_Series_QC_OSD.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6193_SERIES_QC_OSD__
...
#define _ONE_1BIT_ADDRESS_SIZE                          9     // 一個Font等於9個Address
#define _SRAM_SIZE_2BIT_SIZE                            4     // 四個2BitFont
#define _SRAM_SIZE_4BIT_SIZE                            4     // 四個4BitFont

#define _SRAM_SIZE_2BIT_OFFSET                          508   // 為了可以整除2
#define _SRAM_SIZE_4BIT_OFFSET                          508     // 為了可以整除4
#define _SRAM_SIZE_FONT_BASE_ADDRESS                    (21888 - (_SRAM_SIZE_4BIT_OFFSET * 4 * _ONE_1BIT_ADDRESS_SIZE) - (_SRAM_SIZE_4BIT_SIZE * 4 * _ONE_1BIT_ADDRESS_SIZE))  // SRAM 最大size

#define _QC_OSD_POSITION_OFFSET_H                                  0x31
...
BYTE code tQC_OSD_CRC_TABLE_8[] = {
    0xA4, 0x51, 0x49, 0xD3, 0xAB, 0x1A, // M1
    0x43, 0x90, 0x43, 0xFB, 0xA6, 0x61, // M2
};
...
BYTE code tQC_OSD_CRC_TABLE_17[] = {
    0x0C, 0x71, 0x4E, 0x77, 0xCF, 0x15, // M1
    0x7F, 0x7A, 0x02, 0x62, 0x3F, 0xA , // M2
};

BYTE code tQC_OSD_CRC_TABLE_18[] = {
    0xDB, 0xF1, 0xAA, 0x57, 0x79, 0xCC, // M1
    0x25, 0xE6, 0x8D, 0x66, 0x95, 0x6D, // M2
};
...
BYTE code tQC_OSD_CRC_TABLE_20[] = {
    0xE2, 0x28, 0x84, 0xBF, 0xBC, 0x97, // M1
    0x9E, 0x15, 0x75, 0x4C, 0x4A, 0x4A, // M2
};
...
void ScalerQCOsdFontScaleUp(bit bEnable, BYTE ucTpye, BYTE ucType2);
...
void ScalerQCOsdFuncFrame(BYTE ucFrameStatus)
{
    ...
    switch(ucFrameStatus)
    {
        ...
        case _OSD_FRAME_8:
            ScalerOsd2FontFunction(_DISABLE);
            ScalerQCOsdFontScaleUp(_ENABLE, _OSD_FONT_SCALING_RATIO_150, _OSD_FONT_SCALING_BOUNDARY_NEIGHTBOOR);
        ...
        case _OSD_FRAME_17:
            ScalerOsd2FontFunction(_DISABLE);
            ScalerQCOsdFontScaleUp(_ENABLE, _OSD_FONT_SCALING_RATIO_200, _OSD_FONT_SCALING_BOUNDARY_THIN);
        ...
        case _OSD_FRAME_18:
            ScalerOsd2FontFunction(_DISABLE);
            ScalerQCOsdFontScaleUp(_ENABLE, _OSD_FONT_SCALING_RATIO_250, _OSD_FONT_SCALING_BOUNDARY_FAT);
        ...
        //----------------------------
        case _OSD_FRAME_20:
            ScalerQCOsdFontScaleUp(_ENABLE, _OSD_FONT_SCALING_RATIO_300, _OSD_FONT_SCALING_BOUNDARY_NEIGHTBOOR);
           break;
        default:
            break;
    }
...
void ScalerQCCloseOsdFuncFrame(BYTE ucFrameStatus)
{
    ...
    switch(ucFrameStatus)
    {
        ...
        case _OSD_FRAME_8:
            ScalerQCOsdFontScaleUp(_DISABLE, _OSD_FONT_SCALING_RATIO_300, _OSD_FONT_SCALING_BOUNDARY_NEIGHTBOOR);
        ...
        case _OSD_FRAME_20:
            ScalerQCOsdFontScaleUp(_DISABLE, _OSD_FONT_SCALING_RATIO_250, _OSD_FONT_SCALING_BOUNDARY_FAT);
           break;
...
bit ScalerQCOsdTestProc(void)
{
    ...
    ScalerQCOsdFuncFrame(_OSD_FRAME_20);

    usData = (usData | ((WORD)ScalerQCOSDCRCCompare(_D_DOMAIN_CRC, _D_CRC_AFTER_OD, _M1 | _M2, tQC_OSD_CRC_TABLE_20, 1) << 3));

    ScalerQCCloseOsdFuncFrame(_OSD_FRAME_20);
