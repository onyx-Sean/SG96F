...
bit g_bMHLRx3SentMscDcapRdy = _FALSE;
...
bit ScalerMHLRx3MscReadDevCapabilities(EnumMHLMscOffset enumMSCOffset, BYTE *pucData);
...
void ScalerMHLRx3MscCheckDeviceINT(void)
{
    ...
    if((bit)ScalerGetBit(P68_CE_CBUS_CTRL_2E, _BIT7) == _TRUE)
    {
            ...
            ((bit)ScalerGetDataPortBit(P68_AC_CBUS_CTRL_0C, _MSC_RCHANGE_INT, _MSC_DCAP_CHG) == _TRUE)))
        {
            if(GET_MHL_RX3_MHL_VERSION() == _MHL_VERSION_NULL)
            {
                ...
                if(ScalerMHLRx3MscReadDevCapabilities(_MSC_MHL_VERSION_01, &g_ucMHLRx3MhlVersion) == _TRUE)
                {
            ...
            {
                if((ScalerMHLRx3MscReadDevCapabilities(_MSC_ADOPTER_ID_H_03, &g_ucMHLRx3AdopterIDH) == _TRUE) &&
                   (ScalerMHLRx3MscReadDevCapabilities(_MSC_ADOPTER_ID_L_04, &g_ucMHLRx3AdopterIDL) == _TRUE))
                {
        ...
        //------------------------------------//
        if((bit)ScalerGetDataPortBit(P68_AC_CBUS_CTRL_0C, _MSC_RCHANGE_INT, _MSC_REQ_WRT) == _TRUE)
        {
        ...
        // Clear Flag
        ScalerSetByte(P68_CE_CBUS_CTRL_2E, (BYTE)_BIT7);
    }
    ...
    if((bit)ScalerGetBit(P68_CE_CBUS_CTRL_2E, _BIT6) == _TRUE)
    {
        // Read Source Adopter ID
        if((GET_MHL_RX3_ADOPTER_ID_DONE() == _FALSE) && (GET_MHL_RX3_READY_TO_TRANSMIT() == _TRUE))
        {
               ...
               (ScalerMHLRx3MscReadDevCapabilities(_MSC_ADOPTER_ID_L_04, &g_ucMHLRx3AdopterIDL) == _TRUE))
            {
                ...
                ScalerSetBit(P68_CE_CBUS_CTRL_2E, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT6);
            }
        }
    }
