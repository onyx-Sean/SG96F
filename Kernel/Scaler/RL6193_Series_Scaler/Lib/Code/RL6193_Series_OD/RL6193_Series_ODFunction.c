...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6193_Series_ODFunction.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
#include "RL6193_Series_ODLibInternalInclude.h"
...
{
    // Even/odd mode
    WORD usHeightOfEachPath = usHeight / _OD_PATH_NUM;

    // Calculate total size of a frame (in each path) to be saved in memory
    PDATA_DWORD(0) = ((DWORD)CEILING(usWidth, _OD_PIXEL_PER_COMP)) * GET_OD_USE_BIT() / 10 * usHeightOfEachPath;
