...
/********************************************************************************/
//-------------------------------------------------------------------------------------//
// ID Code      : RL6193_Series_ColorGlobalHueSat.c
// Update Note  :
//-------------------------------------------------------------------------------------//
...
void ScalerColorSRGBSetGlobalHueSat(SWORD shDeltaHue, WORD usDeltaSat)
{
    SWORD pshTempBuff[3][3];
    memset(pshTempBuff, 0, sizeof(pshTempBuff));

    if(_TRUE)
    {
        SWORD pshMATRIX[3][3];
        memset(pshMATRIX, 0, sizeof(pshMATRIX));
        ...
        pshMATRIX[0][2] = 0;
        pshMATRIX[1][0] = 0;
        pshMATRIX[1][1] = (DWORD)usDeltaSat * 4096 / 100;
        pshMATRIX[1][2] = 0;
        pshMATRIX[2][0] = 0;
        pshMATRIX[2][1] = 0;
        pshMATRIX[2][2] = pshMATRIX[1][1];

        ScalerColorSRGBMatrixMultiply(pshMATRIX, tRGB2YUV, pshTempBuff);

        pshMATRIX[0][0] = 4096;
        pshMATRIX[0][1] = 0;
        pshMATRIX[0][2] = 0;
        pshMATRIX[1][0] = 0;
        pshMATRIX[1][1] = ScalerColorSRGBCosine(shDeltaHue);
        pshMATRIX[1][2] = (-1) * ScalerColorSRGBSine(shDeltaHue);
        pshMATRIX[2][0] = 0;
        pshMATRIX[2][1] = (-1) * pshMATRIX[1][2];
        pshMATRIX[2][2] = pshMATRIX[1][1];

        ScalerColorSRGBMatrixMultiply(pshMATRIX, pshTempBuff, pshTempBuff);
    }

    ScalerColorSRGBMatrixMultiply(tYUV2RGB, pshTempBuff, pshTempBuff);
    ...
    if(_TRUE)
    {
        ...
        for(pData[0] = 0; pData[0] < 3; pData[0]++)
        {
            for(pData[1] = 0; pData[1] < 3; pData[1]++)
            {
                if(pData[0] == pData[1])
                {
                    pshTempBuff[pData[0]][pData[1]] -= 4096; // 12bit data = 4096; 6369
                }

                if(pshTempBuff[pData[0]][pData[1]] >= 0)
                {
                    pshTempBuff[pData[0]][pData[1]] = ((pshTempBuff[pData[0]][pData[1]] + 4) / 8);
                }
                else
                {
                    pshTempBuff[pData[0]][pData[1]] = ((pshTempBuff[pData[0]][pData[1]] - 4) / 8);
                ...
                if(pshTempBuff[pData[0]][pData[1]] > 2047)
                {
                    pshTempBuff[pData[0]][pData[1]] = 2047;
                }
                else if(pshTempBuff[pData[0]][pData[1]] < -2048)  // 2.9 = 2048
                {
                    pshTempBuff[pData[0]][pData[1]] = -2048;
                }

                // Set sign byteXXH
                pucColorMatrix[2 * (3 * pData[0] + pData[1])] = (pshTempBuff[pData[0]][pData[1]] < 0) ? 1 : 0;  // [0] sign
                pucColorMatrix[2 * (3 * pData[0] + pData[1])] += (((WORD)pshTempBuff[pData[0]][pData[1]] & 0x07) << 5); // [7:5] bit2~0

                // Set data byte XXL
                pucColorMatrix[2 * (3 * pData[0] + pData[1]) + 1] = ((WORD)pshTempBuff[pData[0]][pData[1]] >> 3) & 0xFF;// [7:0] bit10~3
