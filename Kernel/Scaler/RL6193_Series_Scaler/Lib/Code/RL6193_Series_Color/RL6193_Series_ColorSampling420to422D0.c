...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6193_Series_ColorSampling420to422D0.c
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
//****************************************************************************
bit ScalerColor420To422D0Recheck(void);
bit ScalerColor420to422D0CheckCRC(void);
...
bit ScalerColor420To422D0Recheck(void)
{
    ...
    else
    {
        ...
        ScalerSetBit(P2_A6_TMDS_OUTCTL, ~(_BIT6 | _BIT5 | _BIT4), 0x00);

        // enable background color R=0xff G=0x80 B=0x00
        ScalerSetByte(P2_BA_TMDS_ROUT_HIGH_BYTE, 0xFF);
        ScalerSetByte(P2_BC_TMDS_GOUT_HIGH_BYTE, 0x80);
    ...
    for(usCount = 0; usCount < 50; usCount++)
    {
        ...
        if(ScalerColor420to422D0CheckCRC() == _FALSE)
        {
            // 420to422 retoggle
            ...
            {
                ScalerSetByte(P31_20_YUV420_TO_YUV422_D0, 0x83);
                ScalerSetByte(P31_20_YUV420_TO_YUV422_D0, 0x8B);
            }
        }
        else
        {
    ...
    ScalerTMDSRx0SetPorchColor(_COLOR_SPACE_YCBCR420);
