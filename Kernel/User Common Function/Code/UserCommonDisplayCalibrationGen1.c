...
#include "RL6193_Series_Common_RegCommonInclude.h"
...
//--------------------------------------------------
#define _DISP_CALIB_VERSION         1
#define _HDR_TOTAL_IG               8
...
WORD code tHDRIGAMMA_INDEX[] =
{
    ...
    _OGC_HDR_IG_540_NOR_ADDRESS,
    _OGC_HDR_IG_540_DARK_ADDRESS,
};
...
WORD g_usUniformityLUTSavePos = 0x0000; // v01
#endif
WORD g_usColorMatrixSavePos;

BYTE g_ucCalibrationType;
BYTE g_ucUniformityChecksumType = 0x0000; // level0
...
//****************************************************************************************//
        ...
        else if(ucCalibrationStep == _DDCCI_DISP_CALIB_SET_HDR_MODE)
        {
            ...
            ScalerColorPCMInputGammaAdjust(g_pucDISP_CALIBRATION_FLASH + _OGC_HDR_IG_1200_NOR_ADDRESS + (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_SET_HDR_IG_INDEX] * _OGC_HDR_IG_TABLE_SIZE), _OGC_FLASH_BANK);
            ...
            // Set Color Matrix
            ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_HDR_SRGB_MATRIX_ADDRESS + (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_SET_HDR_COLOR_MATRIX] * _OCC_COLORMATRIX_TABLE_SIZE), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
        ...
        {
            UserInterfaceAdjustMaxBacklight();
        }
...
//****************************************************************************************//
        ...
        {
            ScalerDDomainBackgroundEnable(_OFF);
        }
        ...
        else if(ucCalibrationStep == _DDCCI_DISP_CALIB_UNIFORMITY_VERIFY_START)
        {
            if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_UNIFORMITY_INDEX] == _ON)
            {
                ...
                {
                    ScalerColorPanelUniformityOffsetAdjust(g_pucDISP_CALIBRATION_FLASH + tUNIFORMITY_TYPE_OFFSET_ADDRESS_INDEX[ucUniformityTypeIndex], _PANEL_UNIFORMITY_OFFSET_LUT_SIZE, tUNIFORMITY_TYPE_OFFSET_BANK_INDEX[ucUniformityTypeIndex]);
...
#if(_HDR_DICOM_TWO_BRI_CALIBRATION == _ON)
    ...
    switch(ucCheckType)
    {
        ...
        case _OCC_HDR_TYPE:
            ...
            for(usJ = 0; usJ < 3; usJ++)
            {
                for(usI = 0; usI < 148; usI++)
                {
                    UserCommonFlashRead(_OGC_FLASH_BANK, ((((WORD)_OGC_FLASH_PAGE) << 12) + _OCC_HDR_GAMMA10_ADDRESS + usJ * 2200 + usI), 1, pData + 2);
                    pData[0] += pData[2];
                }

                for(usI = 164; usI < 2200; usI = usI + 8)
                {
                    UserCommonFlashRead(_OGC_FLASH_BANK, ((((WORD)_OGC_FLASH_PAGE) << 12) + _OCC_HDR_GAMMA10_ADDRESS + usJ * 2200 + usI), 2, pData + 2);
                    pData[0] += pData[2];
                    pData[0] += pData[3];
                }
            }
...
void UserCommonDdcciDispCalibReplyResult(BYTE ucType)
{
    ...
    switch(ucType)
    {
        case _DISP_CALIB_START:
            ...
            g_pucDdcciTxBuf[_DDCCI_SOURCE] = _DDCCI_DEST_ADDRESS;
            g_pucDdcciTxBuf[_DDCCI_LENGTH] = _DDCCI_CONTROL_STATUS_FLAG | 0x04;
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_OGCOCCDICOM_SUPPORT] = (_HDR_DICOM_TWO_BRI_CALIBRATION << 5) | (_ULTRA_HDR_SUPPORT << 4) | (_OGC_DICOM_SUPPORT << 2) | (_OCC_SUPPORT << 1) | (_OGC_SUPPORT);
...
//--------------------------------------------------
// Description  : Save OCC / OGC Header
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void UserCommonDdcciDispCalibSaveHeader(void)
