...
bit UserCommonDdcciDispCalibOGCStart(void)
{
    ...
    {
        g_pusGammaCount[ucCnt][0] = 387;
        g_pusGammaCount[ucCnt][1] = 387;
        g_pusGammaCount[ucCnt][2] = 387;
    }
#endif  // End of #if(_OGC_SUPPORT == _ON)

#if(_OCC_SUPPORT == _ON)
    g_pusGammaCount[ucCnt][0] = 411;
    g_pusGammaCount[ucCnt][1] = 411;
    g_pusGammaCount[ucCnt][2] = 411;
    ucCnt++;
#endif  // End of #if(_OCC_SUPPORT == _ON)
...
void UserCommonDdcciDispCalibSaveLUT(BYTE ucTotalLength)
{
    ...
    while(ucPointer < (ucTotalLength - 1))
    {
        ...
        UserCommonFlashWrite(_OGC_FLASH_BANK, ((WORD)_OGC_FLASH_PAGE << 12) + (usStartAddr + g_pusGammaCount[ucGammaIndex][ucRGB] - ucLUTLength), ucLUTLength, &g_pucDdcciRxBuf[ucPointer]);
        g_pusGammaCount[ucGammaIndex][ucRGB] = g_pusGammaCount[ucGammaIndex][ucRGB] - ucLUTLength;
