...
void UserInterfaceAdjustMaxBacklight(void);
#endif
...
void UserInterfaceTimerEventProc(EnumScalerTimerEventID enumEventID)
{
	...
	case _USER_TIMER_EVENT_OSD_TIMER:
		        g_usTimeUse++;
        ScalerTimerActiveTimerEvent(SEC(0.991), _USER_TIMER_EVENT_OSD_TIMER);// 

            if((GET_OSD_STATE() == _MENU_NONE)&&(g_MiaoZhun_OnOff == _OFF))
        {
			if(g_usTime_OSD_OnOff1==_ON)
			{
					if((SysModeGetModeState() == _MODE_STATUS_ACTIVE) && (Time_Flag ==0))
					{
        ...
        case _USER_TIMER_EVENT_OSD_DISABLE1:
            OsdDispDisableOsd();
...
#if(_ULTRA_HDR_SUPPORT == _ON)
    if((GET_OSD_PCM_STATUS() != _PCM_OSD_NATIVE) || (GET_OSD_HDR_MODE() == _HDR_MODE_SMPTE_ST_2084))
#else
...
#if(_ULTRA_HDR_SUPPORT == _ON)
//--------------------------------------------------
// Description  : Initial HDR Auto, SMPTE_ST_2084 Mode using in UserInterfaceAdjustColorProc
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void UserInterfaceAdjustHDRColorProcInitial(void)
{
    if(GET_OSD_HDR_MODE() == _HDR_MODE_AUTO)
    {
        if(GET_HDR_EOTF() == _HDR_INFO_EOTF_SMPTE_ST_2084)
        {
            UserCommonAdjustHDRAutoMode(GET_OSD_DARK_ENHANCE_STATUS());
        ...
        }// End of if(GET_HDR_EOTF() == _HDR_INFO_EOTF_SMPTE_ST_2084)
    }
    else if(GET_OSD_HDR_MODE() == _HDR_MODE_SMPTE_ST_2084)
    {
        UserInterfaceAdjustHDRTargetHDR();
    }
}
...
//--------------------------------------------------
// Description  : Get OSD HDR Mode Status Enable
// Input Value  : None
// Output Value : HDR Enable ON/OFF
//--------------------------------------------------
bit UserInterfaceGetHDREnableStatus(void)
{
...
void UserInterfaceAdjustHDR2084(void)
{
    UserAdjustHDR2084(_HDR_MAX_MASTERING_LV_TYPE_1200, _HDR_COLOR_MATRIX_BT2020, GET_OSD_DARK_ENHANCE_STATUS());
}
...
void UserInterfaceAdjustHDRTargetHDR(void)
{
    ...
    if(ScalerColorGetColorSpaceRGB(ScalerColorGetColorSpace()) == _TRUE)
    {
        ...
        SysModeColorSpaceConvert(ScalerColorGetColorSpace(), _DB_APPLY_NO_POLLING);
    }
#endif

#if(_CONTRAST_SUPPORT == _ON)
    UserAdjustHdrContrast(GET_OSD_HDR_CONTRAST());
#endif

    UserAdjustBacklight(_BACKLIGHT_MAX);

#if(_LOCAL_DIMMING_SUPPORT == _ON)
    if(GET_OSD_LOCAL_DIMMING() == _TRUE)
    {
        // local dimming initial
        UserCommonLocalDimmingEnableSetting(_ENABLE);
    }
#endif
}
...
// Input Value  : None
// Output Value : OSD HDR Dark Enahnce ON/OFF
//--------------------------------------------------
bit UserInterfaceGetHDRDarkEnhanceStatus(void)
{
    return GET_OSD_DARK_ENHANCE_STATUS();
}
#endif
...
#if(_ULTRA_HDR_SUPPORT == _ON)
    if((UserCommonAdjustHDREnableStatus(GET_OSD_HDR_MODE()) == _ON) && (GET_OSD_HDR_SHARPNESS() == _ON))
    {
...
void UserInterfaceAdjustUltraVividProc(EnumDBApply enumDBApply)
{
    ...
    if((UserCommonAdjustHDREnableStatus(GET_OSD_HDR_MODE()) == _ON) && (GET_OSD_HDR_SHARPNESS() == _ON))
    {
        UserCommonAdjustUltraVivid(_FUNCTION_ON, ScalerColorGetColorSpace(), enumDBApply);
    }
    else
#endif
    {
        if(GET_OSD_COLOR_FORMAT() == _COLOR_SPACE_RGB)
        {
            ...
            {
                UserCommonAdjustUltraVivid(_FUNCTION_OFF, _COLOR_SPACE_RGB, enumDBApply);
            }
            else
            {
                UserCommonAdjustUltraVivid(_FUNCTION_ON, _COLOR_SPACE_RGB, enumDBApply);
            }
        }
        else
        {
            if(GET_OSD_ULTRA_VIVID_STATUS() == _ULTRA_VIVID_OFF)
            {
                UserCommonAdjustUltraVivid(_FUNCTION_OFF, _COLOR_SPACE_YPBPR, enumDBApply);
            }
            else
            {
                UserCommonAdjustUltraVivid(_FUNCTION_ON, _COLOR_SPACE_YPBPR, enumDBApply);
            }
        }
    }
}
...
#if(_ULTRA_HDR_SUPPORT == _ON)
    if(GET_OSD_HDR_MODE() == _HDR_MODE_SMPTE_ST_2084)
    {
        UserAdjustHDR2084LoadTable(_HDR_MAX_MASTERING_LV_TYPE_1200, _HDR_COLOR_MATRIX_BT2020, GET_OSD_DARK_ENHANCE_STATUS());
        DebugMessageHDR("Load HDR 2084 Table", 0x00);
    }
    else
#endif
    {
...
void UserInterfaceAdjustOsdResetProc(void)
{
    ...
    ScalerTimerCancelTimerEvent(_USER_TIMER_EVENT_SHOW_BURNIN);
}
...
#endif
            return   _DIGITAL_AUDIO;//GET_OSD_AUDIO_SOURCE();
#else
            return _DIGITAL_AUDIO;
#endif // End of #if(_LINE_IN_SUPPORT == _ON)
    }
...
EnumAudioDACOutput UserInterfaceGetAudioDACOutput(void)
{
	...
	{
		return _AUDIO_DAC_OUTPUT_BOTH;// _AUDIO_DAC_OUTPUT_SPEAKER;  //Foley add @20160406
	}
...
//--------------------------------------------------
// Description  : User function Backlight Adjust before HDR and Dicom
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void UserInterfaceAdjustMaxBacklight(void)
...
// Output Value : OSD HDR MODE OFF/AUTO/_HDR_MODE_SMPTE_ST_2084
//--------------------------------------------------
bit UserInterfaceGetLocalDimmingOSD(void)
{
    return GET_OSD_LOCAL_DIMMING();
}
#endif
