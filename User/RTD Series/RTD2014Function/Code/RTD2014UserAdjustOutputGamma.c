...
void UserAdjustGamma(BYTE ucGamma)
{
    ...
    {
#if(_OGC_SUPPORT == _ON)
        ScalerColorOutputGammaAdjust(g_pucDISP_CALIBRATION_FLASH + tGAMMA_OSD_INDEX[ucGamma - 1], _OGC_FLASH_BANK);
        ...
        ScalerColorOutputGammaAdjust(tGAMMA_TABLE[ucGamma - 1], GET_CURRENT_BANK_NUMBER());
...
void UserAdjustHDR2084(EnumHDRMaxMasteringLvType enumHDRMaxMasteringLvType, EnumHDRColorMatrix enumHDRColorMatrix, bit bDarkEnhanceEnable)
{
	EnumHDRMaxMasteringLvType   enumHDRMaxMasteringLvType1=enumHDRMaxMasteringLvType;
	EnumHDRColorMatrix     enumHDRColorMatrix1=enumHDRColorMatrix;
	bit bDarkEnhanceEnable1=bDarkEnhanceEnable;

    ScalerTimerWaitForEvent(_EVENT_DEN_STOP);
#if(_LOCAL_DIMMING_SUPPORT == _ON)
    if(UserInterfaceGetLocalDimmingOSD() == _ON)
    {
	...
	ScalerColorPCMInputGammaAdjust(g_pucDISP_CALIBRATION_FLASH + _OGC_HDR_IG_540_NOR_ADDRESS , _OGC_FLASH_BANK);
