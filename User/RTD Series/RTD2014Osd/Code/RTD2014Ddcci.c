...
#if((_OSD_TYPE == _REALTEK_2014_OSD) || (_TWO_CHIP_DATA_COMMUNICATION_SUPPORT == _ON))
...
#if((_OSD_TYPE == _REALTEK_2014_OSD) || ((_TRANSLATOR_SUPPORT == _ON) && (_TWO_CHIP_TRANSLATOR_CHIP_SHOW_PICTURE_SUPPORT == _ON)))
        ...
        if(GET_RUN_DDCCI_COMMAND() == _SUCCESS)
        {
            switch(g_pucDdcciRxBuf[_DDCCI_COMMAND])
            {
                ...
                case _DDCCI_CMD_SET_VCP_FEATURE:
                    ...
                    // Bypass DDC-CI Command to Downstream Device
                    UserCommonDdcciCmdBypassToDownDevice();
#endif
...
void RTDDdcciGetVCPFeature(void)
{
    ...
    switch(g_pucDdcciRxBuf[_DDCCI_SOURCE_OPCODE])
    {
        ...
        case _DDCCI_OPCODE_VCP_PRODUCTKEY:
                              ...
                              UserCommonFlashRead(_RX_HDCP_1_4_KEY_FLASH_BANK,_RX_HDCP_1_4_KEY_ADDRESS_FLASH+256*g_pucDdcciRxBuf[4]+g_pucDdcciRxBuf[5], 8, &g_pucDdcciTxBuf[3]);
                      #else
                             #warning "Please set _FLASH_ACCESS_FUNCTION_SUPPORT     _ON!!"
                      #endif
                  #elif(_HDCP_1_4_KEY_LOCATION == _TABLE_LOCATION_FLASH)
                      #if(_FLASH_ACCESS_FUNCTION_SUPPORT == _ON)
                              UserCommonFlashRead(_RX_HDCP_1_4_KEY_FLASH_BANK,_RX_HDCP_1_4_KEY_ADDRESS_FLASH+256*g_pucDdcciRxBuf[4]+g_pucDdcciRxBuf[5], 8, &g_pucDdcciTxBuf[3]);
                      #endif
                  #elif(_HDCP_1_4_KEY_LOCATION == _TABLE_LOCATION_SYSTEM_EEPROM)
                      #if(_SYSTEM_EEPROM_EMULATION_SUPPORT == _OFF)
                              UserCommonEepromRead(_RX_HDCP_1_4_KEY_ADDRESS_EEPROM+256*g_pucDdcciRxBuf[4]+g_pucDdcciRxBuf[5], 8, &g_pucDdcciTxBuf[3]);
                      #endif
                  #elif(_HDCP_1_4_KEY_LOCATION == _TABLE_LOCATION_CODE)
                       //reserved
                       //memcpy(&g_pucDdcciTxBuf[3],&tTX_HDCP_KEY_TABLE[256*g_pucDdcciRxBuf[4]+g_pucDdcciRxBuf[5]],  8);
           ...
           #else // #if(_DIGITAL_PORT_SUPPORT == _ON)
    ...
    else
    {
        switch(g_pucDdcciRxBuf[_DDCCI_SOURCE_OPCODE])
        {
           ...
           case _DDCCI_OPCODE_VCP_PRODUCTKEY:
               #if((_DIGITAL_PORT_SUPPORT == _ON))
                        #if(_HDCP_1_4_KEY_LOCATION == _TABLE_LOCATION_USER)
                           if(((WORD)(256*g_pucDdcciRxBuf[4])+g_pucDdcciRxBuf[5]) == 0)
                           {
                           UserCommonFlashErasePage(_RX_HDCP_1_4_KEY_FLASH_BANK, _RX_HDCP_1_4_TABLE_FLASH_PAGE);
                           UserCommonFlashWrite(_RX_HDCP_1_4_KEY_FLASH_BANK,_RX_HDCP_1_4_KEY_ADDRESS_FLASH+256*g_pucDdcciRxBuf[4]+g_pucDdcciRxBuf[5], 8, &g_pucDdcciRxBuf[6]);
                           }
                           else
                           {
                           UserCommonFlashWrite(_RX_HDCP_1_4_KEY_FLASH_BANK,_RX_HDCP_1_4_KEY_ADDRESS_FLASH+256*g_pucDdcciRxBuf[4]+g_pucDdcciRxBuf[5], 8, &g_pucDdcciRxBuf[6]);
                           ...
                           UserCommonEepromWrite(_RX_HDCP_1_4_KEY_ADDRESS_EEPROM+256*g_pucDdcciRxBuf[4]+g_pucDdcciRxBuf[5], 8, &g_pucDdcciRxBuf[6]);
                        #elif(_HDCP_1_4_KEY_LOCATION == _TABLE_LOCATION_CODE)
                       //reserved
                       //memcpy(&tTX_HDCP_KEY_TABLE[256*g_pucDdcciRxBuf[4]+g_pucDdcciRxBuf[5]],&g_pucDdcciRxBuf[6],  8);
                        #endif
...
#endif // End of #if((_OSD_TYPE == _REALTEK_2014_OSD) || ((_TRANSLATOR_SUPPORT == _ON) && (_TWO_CHIP_TRANSLATOR_CHIP_SHOW_PICTURE_SUPPORT == _ON)))
#endif // End of #if((_OSD_TYPE == _REALTEK_2014_OSD) || (_TWO_CHIP_DATA_COMMUNICATION_SUPPORT == _ON))
