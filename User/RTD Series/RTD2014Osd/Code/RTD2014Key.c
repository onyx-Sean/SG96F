    ...
    if((g_ucKeyStateCurr & _POWER_KEY_MASK) != 0)
    {
        if(((g_ucKeyStatePrev ^ g_ucKeyStateCurr) & _POWER_KEY_MASK) != 0)
        {
            ...
            else
            {
				...
				//	ScalerTimerReactiveTimerEvent(SEC(0), _USER_TIMER_EVENT_OSD_DISABLE_HOTKEY_CR_MSG);
...
void RTDKeyPowerKeyMix(void)
{
    switch(g_ucKeyStateCurr)
    {
        ...
        SET_OSD_FACTORY_STATUS(_ON);
        SET_OSD_BURN_STATUS(_ON);
                    RTDNVRamSaveOSDData();
    ...
    else // Key the same
    {
        if(GET_KEYREPEATENABLE() == _TRUE)
        {
            ...
            else
            {
                ...
                ScalerTimerActiveTimerEvent(SEC(1), _USER_TIMER_EVENT_KEY_REPEAT_ENABLE);
            }
        }
    }
}
