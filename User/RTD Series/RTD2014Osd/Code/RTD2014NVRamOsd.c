...
#define _CONTRAST_VGA                               _CONTRAST_CENTER
#define _CONTRAST_DVI                               _CONTRAST_CENTER
#define _CONTRAST_VIDEO8                            _CONTRAST_CENTER
#define _CONTRAST_YPBPR                             _CONTRAST_CENTER
#define _CONTRAST_HDMI                              _CONTRAST_CENTER
#define _CONTRAST_DP                                _CONTRAST_CENTER
#else
#define _CONTRAST_VGA                               0x080
#define _CONTRAST_DVI                               0x080
#define _CONTRAST_VIDEO8                            0x080
#define _CONTRAST_YPBPR                             0x080
#define _CONTRAST_HDMI                              0x080
#define _CONTRAST_DP                                0x080
#endif

#if(_BRI_TYPE == _BRI_GEN_1_10BIT)
#define _BRIGHTNESS_VGA                             0x200
#define _BRIGHTNESS_DVI                             0x200
#define _BRIGHTNESS_VIDEO8                          0x200
#define _BRIGHTNESS_YPBPR                           0x200
#define _BRIGHTNESS_HDMI                            0x200
#define _BRIGHTNESS_DP                              0x200
#else
#define _BRIGHTNESS_VGA                             0x080
#define _BRIGHTNESS_DVI                             0x080
#define _BRIGHTNESS_VIDEO8                          0x080
#define _BRIGHTNESS_YPBPR                           0x080
...
{
    50,// ((((DWORD)((_BACKLIGHT_MAX - _BACKLIGHT_MIN)*10)/10)*8)/10+ _BACKLIGHT_MIN),                   // usBackLight;
...
//--------------------------------------------------------
    _ENGLISH,                 // b4Language;
    _COLOREFFECT_USER,    // b4ColorEffect;
//--------------------------------------------------------
    _CT_USER,                 // b4ColorTempType;
    _COLOR_SPACE_RGB,         // b3VGARGBYUV;
    _COLOR_SPACE_RGB,         // b3DVIRGBYUV;
//--------------------------------------------------------
    _LEVEL_STRONGEST,                        // b3Sharpness;
...
//--------------------------------------------------------
    ...
    _DP_LANE_AUTO_MODE,        // BYTE b3D0DpLaneMode;
    _DP_LANE_AUTO_MODE,        // BYTE b3D1DpLaneMode;
    _DP_LANE_AUTO_MODE,        // BYTE b3D2DpLaneMode;
    _DP_LANE_AUTO_MODE,        // BYTE b3D6DpLaneMode;
//--------------------------------------------------------
  _DP_VER_1_DOT_2,//  _DP_VER_1_DOT_2,                         // b1OsdDpD0PortVersion : 1;
 _DP_VER_1_DOT_2,//   0,                         // b1OsdDpD1PortVersion : 1;
    _OFF,                      // b2OsdDpMST : 2;
    _DISP_ROTATE_0,                         // b3OsdDispRotate : 3;
...
_ECO_MODE_STANDARD, //b3ECOMode;
_LEVEL_OFF,   //b3LowBlue
_AIM_1,
_TIMER_10,
_LEFT_AIM,
_LEFT_TIMER_POSITION_1,
_OFF,
_OFF,//b1FACTORYSTATUS
_ON,//b1BURNSTATUS
_ON,
};
...
#if 0
WORD g_uc4500KColorTempR = _CT4500_RED;
WORD g_uc4500KColorTempG =  _CT4500_GREEN;
WORD g_uc4500KColorTempB = _CT4500_BLUE;
#endif
...
{
ucSource =ucSource;
    UserCommonEepromRead(_BRICON_DATA_ADDRESS + 0 * sizeof(StructBriConDataType), sizeof(StructBriConDataType), (BYTE *)(&g_stBriConData));
}
...
ucSource =ucSource;
    ...
    UserCommonEepromWrite(_BRICON_DATA_ADDRESS + 0 * sizeof(StructBriConDataType), sizeof(StructBriConDataType), (BYTE *)(&g_stBriConData));
}
...
void RTDEepromLoadColorSetting(BYTE ucColorTempType)
{
    ...
    {
#if(_CTS_TYPE == _CTS_GEN_1_12BIT)
        UserCommonFlashRead(_OGC_FLASH_BANK, ((WORD)_OGC_FLASH_PAGE << 12) | (ucColorTempType * _OGC_TOTAL_GAMMA * 6 + (GET_OSD_GAMMA() - 1) * 6), 6, pData);
...
{
    BYTE ucTemp = 0;
    for(ucTemp = _CT_6500; ucTemp < _CT_USER; ucTemp++)
    {
...
void RTDEepromRestoreUserColorSetting(void)
{
    ...
    for(ucIndex = _CT_6500; ucIndex <= _CT_USER; ++ucIndex)
    {
...
void RTDFlashStartup(void)
{
    ...
    else if(ucVersionCode != _VERSION_CODE)
    {
        ...
        g_stColorProcData = tColorTempDefaultData[_CT_USER];
        if(UserCommonFlashLoadItem(_FLASH_ITEMID_COLORPROC2_6500 + _CT_USER, pData, 1) == _TRUE)
        {
...
#endif
    {
        if(UserCommonFlashLoadItem(_FLASH_ITEMID_COLORPROC2_6500 + ucColorTempType, pData, sizeof(StructColorProcDataType)) == _TRUE)
        {
...
void RTDFlashSaveColorSetting(BYTE ucColorTempType)
{
    UserCommonFlashSaveItem(_FLASH_ITEMID_COLORPROC2_6500 + ucColorTempType, (BYTE *)(&g_stColorProcData), sizeof(StructColorProcDataType));
}
...
void RTDFlashRestoreColorSetting(void)
{
    ...
    for(ucColorTempIndex = 0; ucColorTempIndex <= _CT_COLORTEMP_AMOUNT; ucColorTempIndex++)
    {
        ...
        if(UserCommonFlashLoadItem(_FLASH_ITEMID_COLORPROC2_6500 + ucColorTempIndex, pData, 1) == _TRUE)
        {
